...
import com.mycompany.sistema_MVC.controller.ClienteController;
...
import com.mycompany.sistema_MVC.model.DAO.ClienteDAO;
...
public class ClienteView extends javax.swing.JFrame {
    ...
    //136.573.426-99
     ...
     public void excluirCliente(){
        ...
        if(tblCliente.getSelectedRow() != -1){
            id = (int) tblCliente.getValueAt(tblCliente.getSelectedRow(), 0);
            cliente = buscarCliente(id);
            preencherForm();
            this.habDesabForm(false);
            if(JOptionPane.showConfirmDialog(rootPane, "Deseja realmente excluir o(a) Cliente: "  + cliente[1],  "Sistema Cliente", JOptionPane.YES_NO_OPTION) == 0){
                //arrayCliente.remove(cliente);
                if(clienteController.removerCliente(cliente) > 0){
                    JOptionPane.showMessageDialog(rootPane, "Cliente (" + cliente[1] + ") excluído com Sucesso!");
                    limparDados();
                    this.carregarTblCliente();
                }else
                    JOptionPane.showMessageDialog(rootPane, "Erro ao excluir o Cliente"+ cliente[1] + "!");
            }
        }
     }
    ...
    public Object [] buscarCliente(int id){
        for(int i=0; i < arrayCliente.size(); i++){
            if((int)arrayCliente.get(i)[0] == id)
    ...
    public void preencherForm(){
        ...
        if(tblCliente.getSelectedRow() != -1){
            id = (int) tblCliente.getValueAt(tblCliente.getSelectedRow(), 0);
            cliente = buscarCliente(id);
            this.txtId.setText(cliente[0].toString());
            this.txtNome.setText(cliente[1].toString());
            this.txtIdade.setText(cliente[3].toString());
            this.txtEmail.setText(cliente[4].toString());
            this.pswSenha.setText(cliente[6].toString());
            this.ftxtCpf.setText(cliente[5].toString());
            if(cliente[2].toString().toCharArray()[0] == 'M')
    ...
    public void carregarTblCliente(){
        String [] colunas = {"Id","Nome","Idade", "Sexo", "CPF", "Senha", "Email"};
        DefaultTableModel model = new DefaultTableModel(colunas, 0);
        
        arrayCliente = clienteController.buscarCliente();
        
        for(int i=0; i<arrayCliente.size(); i++){
            Object [] linha = {arrayCliente.get(i)[0],arrayCliente.get(i)[1],arrayCliente.get(i)[2], arrayCliente.get(i)[3], 
            arrayCliente.get(i)[4], arrayCliente.get(i)[5], arrayCliente.get(i)[6]};
            model.addRow(linha);
        }
    ...
    public void salvarCliente(){
        ...
        cliente[0] = txtId.getText();
        cliente[1] = txtNome.getText();
        cliente[2] = txtIdade.getText();
        cliente[3] = cbxSexo.getItemAt(cbxSexo.getSelectedIndex());
        cliente[4] = ftxtCpf.getText();
        cliente[5] = String.valueOf(pswSenha.getPassword());
        cliente[6] = txtEmail.getText();
        
        if(tipoEdicao == 'I')
            if(clienteController.incluirCliente(cliente)>0)
                JOptionPane.showMessageDialog(rootPane, "Cliente cadastrado com Sucesso!");
            else
                JOptionPane.showMessageDialog(rootPane, "Erro ao Cadastrar o Cliente.");
        else
            if(clienteController.modificarCliente(cliente) > 0)
                JOptionPane.showMessageDialog(rootPane, "Cliente alterado com Sucesso!");
            else
                JOptionPane.showMessageDialog(rootPane, "Erro ao alterar o Cliente.");
    ...
    public void consultarCliente(String nome){
        for(int i=0; 1<arrayCliente.size(); i++){
            ...
            if(cliente[1].toString().toUpperCase().contains(nome))
                for(int j=0; j < tblCliente.getRowCount(); j++){
                    if((int)tblCliente.getValueAt(j, 0) == (int)cliente[0]){
